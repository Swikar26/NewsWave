...
// apis 
const API_KEY = "829566100fb3473f9a829c70e13e10f5";
...
const WORLD_NEWS = "https://www.sheltonherald.com/news/world?country=in&category=world&apiKey=";
const DEFENCE_NEWS = "https://newsapi.org/v2/top-headlines?country=in&category=defence&apiKey=";
...
defenceBtn.addEventListener("click", function() {
    newsType.innerHTML = "<h4>Defence</h4>";
    fetchDefenceNews();
});
...
const fetchScineceNews = async() => {
    const response = await fetch(SCINECE_NEWS + API_KEY);
    newsDataArr = [];
    if (response.status >= 200 && response.status < 300) {
        const myJson = await response.json();
        newsDataArr = myJson.articles;
...
const fetchDefenceNews = async() => {
    const response = await fetch(DEFENCE_NEWS + API_KEY);
    newsDataArr = [];
    if (response.status >= 200 && response.status < 300) {
        const myJson = await response.json();
        newsDataArr = myJson.articles;
